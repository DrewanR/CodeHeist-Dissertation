...
\paragraph{Conditional Structures (\texttt{if}, \texttt{elif}, \texttt{else})} As shown in \autoref{fig:design/CatCode-ConditionalFSM}, there are 5 states when interpreting conditional structures. Every state except the initial state \textquote{Evaluate} and the final state \texttt{end} are associated with the pass state of the same name.
