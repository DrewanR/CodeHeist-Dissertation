...
As mentioned before, a full prototype could not be made, nor could a user evaluation be deployed. However, the Core Systems were developed to a high standard (81\%, whilst being harsh). Combined with the Developer's Report, it can be concluded that 8 weeks was not sufficient to develop a full prototype; however, it such amount of time was sufficient to make a basic prototype of CatCode.

This, sadly means that no conclusion can be made about the efficacy of thus approach, but we do believe that the approach does show great potential. We also think that there is demand for a system similar to what we developed.

Returning to our aims and objectives from \autoref{sec:introduction/aims_and_objectives}, we were able to partially fulfil our aim of \textquote{simulating a short development period}, although we were not able evaluate this with users. Objective 1 was fulfilled earlier in this report, even if neither curriculum provided satisfactory answers. Objective 3 was fulfilled alongside objective 2. Only objective 4 wasn't fulfilled due to time constraints.
...
Most of the development of the prototype was based on the developer's subjective, heuristic judgement. Further research into teaching could be conducted to improve the design of CatCode, the wider requirements, and level design. Also, primary research could be conducted with teachers to better develop requirements software to their needs as this paper omitted engagement with stakeholders such as a teachers due to time constraints.
...
Within this report and prototype, we focused on procedural programming across single scripts. There are other programming paradigms or scenarios this approach may would work better for. We also focused on creating a teaching tool for beginners, however this approach may serve better as a revision aid or for non-beginners. 
...
There are a few improvements that can be made to the core systems:
...
\begin{itemize}
    \item An RPG (Roleplaying game), where rather than controlling characters directly, you implement their program their \textquote{AI}\footnote{AI in this case is a term, often used in games, referring to a set of rules that govern how an entity acts.}. This system could be a more developed version of systems that exist in many RPGs where you customise non-player controller party members' behaviour.
    \item Similar to the RPG idea, a rogue-like\footnote{Or rogue-lite in the likely scenario it strays from the Berlin Definition, as described by \citep{destructoidRoguelikeDef}. The exact definition of this genre is a controversial topic.} where items take the form of CatCode. This game could function similar to \citep{SlayTheSpire}, swapping the deck-building for programming where your hand is swapped for a script or scripts.
